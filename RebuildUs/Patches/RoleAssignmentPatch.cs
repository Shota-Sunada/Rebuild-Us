...
class RoleManagerSelectRolesPatch
{
    ...
    private static List<(byte, byte)> playerRoleMap = [];
    ...
    public static RoleAssignmentData getRoleAssignmentData()
    {
        ...
        impSettings.Add((byte)RoleId.Morphing, CustomOptionHolder.morphingSpawnRate.Data);
        ...
        if (rnd.Next(1, 101) <= CustomOptionHolder.lawyerIsProsecutorChance.getSelection() * 10) // Lawyer or Prosecutor
            neutralSettings.Add((byte)RoleId.Prosecutor, CustomOptionHolder.lawyerSpawnRate.Data);
        else
        ...
        crewSettings.Add((byte)RoleId.Trapper, CustomOptionHolder.trapperSpawnRate.Data);
    ...
    private static void assignEnsuredRoles(RoleAssignmentData data)
    {
        ...
        List<byte> ensuredCrewmateRoles = [.. data.crewSettings.Where(x => x.Value.rate == 10).Select(x => x.Key)];
        List<byte> ensuredNeutralRoles = [.. data.neutralSettings.Where(x => x.Value.rate == 10).Select(x => x.Key)];
        List<byte> ensuredImpostorRoles = [.. data.impSettings.Where(x => x.Value.rate == 10).Select(x => x.Key)];
            ...
            )))
        {
            ...
            Dictionary<RoleType, List<byte>> rolesToAssign = [];
    ...
    private static void assignChanceRoles(RoleAssignmentData data)
    {
        ...
        List<byte> crewmateTickets = [.. data.crewSettings.Where(x => x.Value.rate > 0 && x.Value.rate < 10).Select(x => Enumerable.Repeat(x.Key, x.Value.count)).SelectMany(x => x)];
        List<byte> neutralTickets = [.. data.neutralSettings.Where(x => x.Value.rate > 0 && x.Value.rate < 10).Select(x => Enumerable.Repeat(x.Key, x.Value.count)).SelectMany(x => x)];
        List<byte> impostorTickets = [.. data.impSettings.Where(x => x.Value.rate > 0 && x.Value.rate < 10).Select(x => Enumerable.Repeat(x.Key, x.Value.count)).SelectMany(x => x)];
    ...
    public static void assignModifiers()
    {
        ...
        List<RoleId> allModifiers = [];
        List<RoleId> ensuredModifiers = [];
        List<RoleId> chanceModifiers = [];
        ...
        List<RoleId> chanceModifierToAssign = [];
    ...
    {
        modifiers = [.. modifiers.OrderBy(x => rnd.Next())]; // randomize list
