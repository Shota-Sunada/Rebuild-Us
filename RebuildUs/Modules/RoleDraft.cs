...
namespace RebuildUs.Modules
{
    ...
    class RoleDraft
    {
                ...
                while (timer < maxTimer || !picked)
                {
                    ...
                    if (PlayerControl.LocalPlayer.PlayerId == pickOrder[0])
                    {
                        ...
                        if (availableRoles.Count == 0)
                        {
                            ...
                            RebuildUsPlugin.Instance.Logger.LogWarning("Draft Mode: Fallback triggered, because no roles were left. Forced addition of basegame Imp/Crewmate");
                        }
        ...
        public static void receivePick(byte playerId, byte roleId)
        {
            ...
            catch (Exception e) { RebuildUsPlugin.Instance.Logger.LogError(e); }
        }
